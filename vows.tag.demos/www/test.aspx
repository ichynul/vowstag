...
<asp:Repeater ID="ListTag_241_1" runat="server">
    <ItemTemplate><!--不分页3条-->
    <li><a class="title" href="formTag.aspx?id=<%# Eval("ID") %>"><%# SubString(Eval("Title"),15) %></a>
        <span class="time"><%# Eval("Time") %></span> </li>
    </ItemTemplate>
</asp:Repeater>
<asp:Literal ID="empty_ListTag_241_1" runat="server"></asp:Literal>

    <hr />
    <!--label中的read  读取那条ID为4的文章-->
    <a style="color: Red;" href="read.aspx?id=<% =ReadTag_17542_5.ID %>"><% =ReadTag_17542_5.Title %><% = SubString(ReadTag_17542_5.Title,15) %><!--限制标题最多15个字--></a>
<asp:Repeater ID="ListTag_57754_9" runat="server">
    <ItemTemplate><!--不分页3条-->
    <li><a class="title" href="formTag.aspx?id=<%# Eval("ID") %>"><%# SubString(Eval("Title"),15) %></a>
        <span class="time"><%# Eval("Time") %></span> </li>
    </ItemTemplate>
</asp:Repeater>
<asp:Literal ID="empty_ListTag_57754_9" runat="server"></asp:Literal>
