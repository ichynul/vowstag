...
public partial class Page_jsonTag : xx.yy.PageBase
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            ...
            {
                Bind_ListTag_459_6();
                Load_LabelTag_16842_3();
            }
        }
    }

    public void Bind_ListTag_459_6()
    {
        ...
        if (list.Count() == 0)
        {
            empty_ListTag_459_6.Text = "<div class='emptydiv'><span class='emptytext'>暂无内容</span></div>";
            return;
        }
        ListTag_459_6.DataSource = list.ToList();
        ListTag_459_6.DataBind();
    }

    public void Load_LabelTag_16842_3()
    {
        SubControl uc_label=(SubControl) LoadControl( "notice.ascx");
        uc_label.SetDb(this.Db_Context);
        uc_label.SetConfig(this.config);
        LabelTag_16842_3.Controls.Add(uc_label);
    }

    public CallbackResult CallBack_JsonTag_7154_11()
    {
        /*orderby=时间&pagesize=3&fields=title,时间,time,desc,img,id*/
        string jsonname = this.CallValue("jsonname");
        if (jsonname != "JsonTag_7154_11")
        {
            return null;
        }
    ...
    public CallbackResult CallBack_JsonTag_8554_12()
    {
        /*orderby=time&pagesize=3&fields=title,time,desc,img,id&categ=call.cid*/
        string jsonname = this.CallValue("jsonname");
        if (jsonname != "JsonTag_8554_12")
        {
            return null;
        }
    ...
    public override CallbackResult TagCallback()
    {
        ...
        callback = CallBack_JsonTag_7154_11();
        if (callback != null)
        {
            return callback;
        }
        callback = CallBack_JsonTag_8554_12();
        if (callback != null)
        {
            return callback;
        }
